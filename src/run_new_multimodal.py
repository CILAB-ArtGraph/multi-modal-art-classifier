...
parser.add_argument('--emb_node', type=str, default='genre', help='Embedding of which node: (artwork|style|genre).')
...
dataset_train, dataset_valid, dataset_test = load_dataset_embeddings(
    base_dir = args.dataset_path, image_dir = args.image_path, mode = 'single_task', label = args.label, emb_node = args.emb_node, emb_type = args.emb_type)
...
model = NewMultiModalSingleTask(emb_size = 128, num_class = num_classes[args.label], dropout=0)
...
with mlflow.start_run() as run:
    ...
    mlflow.log_param('emb node', args.emb_node)
